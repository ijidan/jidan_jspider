		...
		<form action="" class="frm" rel="async" method="post">
			<table class="data-tbl frm-row-tbl">
				...
				<tr>
					<td>中文名称</td>
					<td>
						<input type="text" class="long-txt" name="product_chinese_name" required value="">
					</td>
					<td></td>
				</tr>
				<tr>
					<td>英文名称</td>
					<td>
						<input type="text" class="long-txt" name="product_english_name" required value="">
					</td>
					<td></td>
				</tr>
				<tr>
				...
				<tr>
					<td>商品链接</td>
					<td>
						<input type="text" class="long-txt" name="product_urls" required value="">(多个用英文逗号,分隔)
					</td>
					<td></td>
				</tr>
				<?php echo $TPL; ?>
	...
	<script>
		seajs.use(["jquery", "ywj/msg", 'ywj/net'], function ($, Msg, net) {
			$("input[type='submit']").click(function () {
				...
				var product_chinese_name = $.trim($("input[name=product_chinse_name]").val());
				...
				net.post("/foreignTradeProduct/addProduct", request_param, function (json) {
					Msg.show("success");
				});
			});
			return false;
